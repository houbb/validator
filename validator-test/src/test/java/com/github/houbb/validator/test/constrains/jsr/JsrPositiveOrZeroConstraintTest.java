...
public class JsrPositiveOrZeroConstraintTest {
    ...
    public void assertFalseTest() {
        IResult result1 = ValidHelper.failOver(-1, Constraints.positiveOrZeroConstraint());
        Assert.assertFalse(result1.pass());

        IResult result2 = ValidHelper.failOver(-1L, Constraints.positiveOrZeroConstraint());
        Assert.assertFalse(result2.pass());

        IResult result3 = ValidHelper.failOver(-1.22, Constraints.positiveOrZeroConstraint());
        Assert.assertFalse(result3.pass());

        IResult result4 = ValidHelper.failOver("-2", Constraints.positiveOrZeroConstraint());
        Assert.assertFalse(result4.pass());

        IResult result5 = ValidHelper.failOver(new BigDecimal("-5"), Constraints.positiveOrZeroConstraint());
        Assert.assertFalse(result5.pass());

        IResult result6 = ValidHelper.failOver(new BigInteger("-7"), Constraints.positiveOrZeroConstraint());
        Assert.assertFalse(result6.pass());
    }

    @Test
    public void assertTrueTest() {
        IResult result0 = ValidHelper.failOver(0, Constraints.positiveOrZeroConstraint());
